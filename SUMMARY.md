...
- **🤖 Auto smart router** (src/core/router.ts, uncommitted): `Web2API: Gemini 3.5 Flash Lite` pre-classifies A/B/C with a 2.5s timeout, heuristic fallback (`classifyHeuristic`), maps A→T0 first, B→T1 first, C→T4 free Web2API. Entry: `/model auto` / keyboard `🤖 Auto` button.
