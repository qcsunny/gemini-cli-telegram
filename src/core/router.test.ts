...
const MOCK_TIERS = [
  ...
  { name: '远程备用', priority: 4, models: ['Web2API: Gemini 3.6 Flash Thinking', 'Web2API: Gemini Flash Lite'] },
];
...
describe('router', () => {
  ...
  describe('resolveModelForCategory', () => {
    ...
    it('should map C to a free tier-4 Web2API model', () => {
      expect(resolveModelForCategory('C')).toBe('Web2API: Gemini 3.6 Flash Thinking');
    });
  });
  ...
  describe('classifyAndRouteQuery', () => {
    ...
    it('should fall back to heuristic when the LLM call throws', async () => {
      ...
      expect(decision.targetModel).toBe('Web2API: Gemini 3.6 Flash Thinking');
