...
function markdownToHtmlSnippet(markdown: string): string {
  ...
  html = html.replace(/<a\s+href="https?:\/\/(.*?)"\s*>([^<]+)<\/a>/gi, (match, url, text) => {
...
export function markdownToHtml(input: string | StructuredMessage, isStreaming = false): string {
  ...
  if (typeof input === 'object') {
    if (!input.content && !input.thought) return '';
    try {
      ...
      if (input.thought && input.thought.trim()) {
        ...
        html = contentHtml.trim()
          ? `${thoughtHtml}<br><br>${contentHtml.trim()}`
          : thoughtHtml;
      }
      ...
      return input.content || input.thought || '';
    }
  }
